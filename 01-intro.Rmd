...
Humans have been interacting with AI chatbots for years, in fact Alan Turing is credited with coming up with the concept for chatbots in 1950. Chatbots are software-based systems that interact with humans typically by text or speech inputs, rather than code and mimic some human activity [@wikipedia_chatbot_2023; @abdulla2022chatbots]. They process the inputs using natural language processing commonly abbreviated as NLP. NLP is a kind of AI that uses human text or speech and parses the language to determine hierarchical structures and patterns to extract meaning. NLP uses large amounts of language data (such as books, websites etc.) to train AI systems to identify these structures and patterns, for example identifying when a sentence is a question or a statement by examining various features in a prompt such as the inclusion of a question mark of the use of words often in questions [@wikipedia_natural_2023, @cahn2017chatbot]. The methods used for chatbots have changed over time. Now chatbots often utilize AI methods like [deep learning](https://en.wikipedia.org/wiki/Deep_learning) (which involve multiple layers of abstractions of the input data [@wikipedia_deep_learning_2023]) to extract meaning from the language data [@wikipedia_natural_2023]. As these methods use large quantities of text, they are therefore often called large language models @wikipedia_large_language_2023].

Despite the fact that chatbots have been around awhile, the popularity of OpenAI's ChatGPT and DALL-E programs has sparked a recent surge of interest. These chatbots are particularly powerful due to the fact that large amounts of computing power were used to train their NLP models on very large datasets [@caldarini2022literature; @cahn2017chatbot]. For example, Google's C4 data set (which was used to train Google's T5 Chatbot) contains the contents of over 15 million websites[@schaul_inside_nodate]! Large language model AIs can be divided into two categories: those that can be reached using an internet browser, and those that can be reached using an integrated development environment (IDE).

```{r, fig.align='center', out.width="100%", echo = FALSE, fig.alt= "The dinosaur cartoon says in a speech bubble 'Wow, there are already so many options!' as names of AI tools swirl around the dinosaur's head."}
ottrpal::include_slide("https://docs.google.com/presentation/d/1MCNeSO4aOm1iESWDLOGTcx3aLEbnu8UttV0QGVAeafE/edit#slide=id.g23e17ee1585_223_77")
```
